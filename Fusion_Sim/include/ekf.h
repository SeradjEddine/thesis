...
void ekf_init_gps_ref(struct fused_gps *gps_first); /* expects fused_gps from multi_fuse */
...
int ekf_update_block(struct ekf_state *x, double P[P_DIM],
                     const double z[3], const double h[3], const double R[9],
                     int state_index, double gate_thresh,
                     double *out_mahalanobis, int *out_accepted);
...
/* Build discrete process noise block (public helper) */
